...
exports.handler = async (event) => {
  ...
  const priceId = process.env.OFFICIAL_PLAYBOOK_STRIPE_PRICE_ID || 'price_1TwA6QASlf43jszV4L67qeoU';
  ...
  params.set('success_url', `${origin}/official-playbook?purchase=success&session_id={CHECKOUT_SESSION_ID}`);
  params.set('cancel_url', `${origin}/official-playbook`);
  ...
  if (!response.ok || !result.url) {
    console.error('Official playbook checkout session failed', result);
